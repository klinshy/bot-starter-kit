...
export default {
    run: async (metadata: any) => { //this is supposed to run only once //somewhere in the code there is a subscribe method, which doesnt get cleaned up, so we need to unsubscribe it. //when self hosting, 1. headless browser, 2. browser needs to trigger the run method -> if player.tag === bot then start the run method
        await WA.onInit();
//add console logs
//try to add a breakpoint in the browser while debugging
